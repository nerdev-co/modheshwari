...
import { ApprovalStatusSchema } from "../lib/sharedSchemas";
...
export async function handleCreateEvent(req: Request): Promise<Response> {
    try {
        ...
        const v = await validateBody(req, CreateEventSchema);
        ...
        const event = await prisma.$transaction(async (tx) => {
            ...
            await createOutboxEvent(tx, {
                ...
                payload: {
                    ...
                    status: ev.status,
                    locationLat:
                        (ev as { locationLat?: number | null }).locationLat ??
                        null,
                    locationLng:
                        (ev as { locationLng?: number | null }).locationLng ??
                        null,
                },
...
export async function handleApproveEvent(
    ...
    try {
        ...
        const v = await validateBody(req, ApproveEventSchema);
        if (!v.ok) return v.response;
        const body = v.data;
