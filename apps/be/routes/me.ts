...
import { isValidBloodGroup, normalizeBloodGroup } from "../utils/searchParser";
    ...
    try {
        ...
        if (!user) {
            ...
            await prisma.profile.create({
                data: { userId, status: true, bloodGroup: "O_POS" },
            });
...
export async function handleUpdateMe(req: Request): Promise<Response> {
    try {
        ...
        const validation = await validateBody(req, UpdateMeSchema);
